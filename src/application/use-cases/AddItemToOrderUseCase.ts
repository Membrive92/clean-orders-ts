...
import { ValidationError, NotFoundError, ConflictError, InfraError } from '../errors/index.js';
...
export class AddItemToOrderUseCase {
  ...
  async execute(dto: AddItemToOrderDTO): Promise<Result<void, AddItemToOrderError>> {
    try {
      // 1. Validar el DTO
      ...
      if (orderResult.value === null) {
        ...
        return fail({ type: 'NOT_FOUND_ERROR' as const, message: notFoundError.message });
      }
