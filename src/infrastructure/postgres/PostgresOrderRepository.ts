 ...
 * Implements UPSERT for orders and DELETE+INSERT for order_items
 */
export class PostgresOrderRepository implements OrderRepository {
  ...
  async save(order: Order): Promise<Result<void, string>> {
    ...
    try {
      ...
      // Delete existing items and insert new ones (simpler than complex upsert logic)
  ...
  /**
   * Find order by ID
   * Reconstructs the Order aggregate from database rows
   */
  async findById(id: string): Promise<Result<Order | null, string>> {
    ...
    try {
      ...
      // Restore status (order is created in DRAFT, need to update if different)
   ...
   * Same implementation as save (uses UPSERT)
   */
  ...
  async findAll(limit?: number, offset?: number): Promise<Result<Order[], string>> {
    ...
    try {
      ...
      const orderResult = await client.query<OrderRow>(query, params);
